...
function ex_ruby_catch_save($post_id) {
  global $post;
...
add_action('save_post', 'ex_ruby_catch_save');
...
?>
  <tr class="form-field">
    ...
    <td><input type="text" name="Cat_meta[catch]" id="catch" size="25" value="<?php if(isset ( $cat_meta['catch'])) echo esc_html($cat_meta['catch']) ?>" /></td>
  </tr>
  <tr class="form-field">
    <th><label for="bzb_meta_ruby">カテゴリーページサブキャッチコピー</label></th>
    <td><input type="text" name="Cat_meta[ruby]" id="ruby" size="25" value="<?php if(isset ( $cat_meta['ruby'])) echo esc_html($cat_meta['ruby']) ?>" /></td>
  </tr>
<?php
}
?>