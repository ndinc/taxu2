...
function my_box_save($post_id) {
  ...
  //if($_POST['post_type'] == 'post'){
    ...
    (isset($_POST['show_toppage_flag'])) ? update_post_meta($post->ID, 'show_toppage_flag', $_POST['show_toppage_flag']) : "";
    (isset($_POST['include_rss'])) ? update_post_meta($post->ID, 'include_rss', $_POST['include_rss']) : "";
    (isset($_POST['post_social_buttons'])) ? update_post_meta($post->ID, 'post_social_buttons', $_POST['post_social_buttons']) : "";
    //
...
?>
  ...
  <label for="bzb_meta_robots1"><input class="cmb_option" type="checkbox" name="bzb_meta_robots[]" id="bzb_meta_robots1" value="noindex"  <?php echo (is_array($metarobots_array) && in_array('noindex', $metarobots_array)) ? 'checked' : '';?> /> noindex</label></li>
  <li>
  <input type="hidden" name="bzb_meta_robots[]" value="">
  <label for="bzb_meta_robots2"><input class="cmb_option" type="checkbox" name="bzb_meta_robots[]" id="bzb_meta_robots2" value="nofollow" <?php echo (is_array($metarobots_array) && in_array('nofollow', $metarobots_array)) ? 'checked' : '';?>  /> nofollow</label></li>
  </ul>
<?php
}
...
<?php
  ...
  if(is_array($bzb_cta)) extract($bzb_cta);

?>
<table class="form-table cmb_metabox">
<tr>
<td><small>記事直下に表示するCTAを選択して下さい。 <a href="https://xeory.jp/xeory-cta" target="_blank">コンバージョンが取れるCTAの作り方</a></small><ul>
<li class="cmb_option"><input type="radio" name="bzb_cta[check_cta]" id="bzb_cta1" value="none" <?php checked($check_cta, "none");?> /> <label for="bzb_cta1">表示しない</label></li>
<li class="cmb_option"><input type="radio" name="bzb_cta[check_cta]" id="bzb_cta2" value="custompost" <?php checked($check_cta, "custompost");?> /> <label for="bzb_cta2">既に作成してあるCTAの中から選ぶ</label></li>
<li class="cmb_option"><input type="radio" name="bzb_cta[check_cta]" id="bzb_cta3" value="pageorg" <?php checked($check_cta, "pageorg");?> /> 
<label for="bzb_cta3">このページ独自のCTAを作る</label></li></ul>
...
<?php cmb_render_cta_select('bzb_cta[cta_select]', $cta_select);?><br><br>
</td></tr>
...
add_filter('cmb_render_cta_select','cmb_render_cta_select', 10, 2 );
function cmb_render_cta_select($field, $meta ){
...
<table class="form-table cmb_metabox"><tr class="cmb-type-multicheck cmb_id_bzb_checklists"><label style="display:none;" for="bzb_checklists">チェックリスト</label></th><td><small>SEO効果を最大化するため、記事更新前に、必要な設定を全てできているかを確認しましょう。</small><ul>
...
<li><input class="cmb_option" type="checkbox" name="bzb_checklists[]" id="bzb_checklists11" value="check11"   <?php echo ( is_array($checklists) && in_array("check11",$checklists))? "checked='checked'":"";?> /> <label for="bzb_checklists11">解決策の根拠を提示している</label></li></ul>
...
function add_show_toppage_flag() {
  add_meta_box('show_toppage_flag', 'TOPページへの表示', 'show_toppage_flag', 'post', 'side', 'low');
}
add_action('add_meta_boxes', 'add_show_toppage_flag');
 
function show_toppage_flag() {  
  global $post;
  $show_toppage_flag = get_post_meta($post->ID, 'show_toppage_flag', true);
  $show_toppage_flag = esc_html($show_toppage_flag);
?>

<input class="cmb_option" type="hidden" name="show_toppage_flag" value="" />
<input class="cmb_option" type="checkbox" name="show_toppage_flag" value="none" <?php checked('none', $show_toppage_flag);?>/>
...
function add_include_rss() {
  add_meta_box('include_rss', 'RSSへの登録', 'include_rss', 'post', 'side', 'low');
}
add_action('add_meta_boxes', 'add_include_rss');
 
function include_rss() {  
  global $post;
  $include_rss = get_post_meta($post->ID, 'include_rss', true);
  $include_rss = esc_html($include_rss);
?>
<input class="cmb_option" type="hidden" name="include_rss" value="" />
<input class="cmb_option" type="checkbox" name="include_rss" value="none" <?php checked('none', $include_rss);?>/>
...
function add_post_social_buttons() {
  add_meta_box('post_social_buttons', 'ソーシャルボタンの表示', 'post_social_buttons', 'post', 'side', 'low');
}
add_action('add_meta_boxes', 'add_post_social_buttons');
 
function post_social_buttons() {  
  global $post;
  $post_social_buttons = get_post_meta($post->ID, 'post_social_buttons', true);
  $post_social_buttons = esc_html($post_social_buttons);
?>
<input class="cmb_option" type="hidden" name="post_social_buttons" value="" />
<input class="cmb_option" type="checkbox" name="post_social_buttons" value="none" <?php checked('none', $post_social_buttons);?>/>
