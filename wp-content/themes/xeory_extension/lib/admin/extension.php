...
<h3 class='hndle'><span>クローズアップ記事の設定</span></h3>
  ...
  $bzb_contents_array = get_option('bzb_contents');
        ...
        //print_r($bzb_contents_array);exit;
...
?>
      ...
      <h4>お問い合わせフォーム本文</h4>
      <p class="setting_description">文章とショートコードを入力します。“ショートコード”とは、お問い合わせフォーム用のコードのことです。詳しくは『<a href="http://bazubu.com/wordpress-inquiry-form-24081.html">問い合わせページを作る方法</p>』をご確認ください。</p>
...
function bzb_make_contents_list(){
  ...
  $list .= '<div style="display:none;" class="page_list"><SELECT size="6" name="bzb_contents[' . '\' + count + \'' . '][page]" class="bzb_post" style="height:auto;">';
        query_posts('post_status=publish&post_type=page');
        while ( have_posts() ) : the_post();
          $list .= '<OPTION value="' . get_the_ID() . '">' . addslashes(get_the_title()) . '</OPTION>';
        //$list .= '<OPTION value="aa">aa</OPTION>';
        endwhile;
  ...
  $list .= "</div>";
  $list .= '<p class="remove2"><input class="remove2" type="button" value="この項目を削除する"></p>';
...
function bzb_show_service_list($no){
  ...
  $list .= '<div style="' . $check_post . '" class="post_list"><SELECT size="6" name="bzb_service[' . $no . '][post]" class="bzb_page" style="height:auto;">';
        query_posts('post_status=publish');
        while ( have_posts() ) : the_post();
          $list .= '<OPTION value="' . get_the_ID() . '"'. selected($post, get_the_ID(), false) . '>' . addslashes(get_the_title()) . '</OPTION>';
        //$list .= '<OPTION value="aa">aa</OPTION>';
  ...
  $list .= '<div style="' . $check_page . '" class="page_list"><SELECT size="6" name="bzb_service[' . $no . '][page]" class="bzb_post" style="height:auto;">';
        query_posts('post_status=publish&post_type=page');
        while ( have_posts() ) : the_post();
          $list .= '<OPTION value="' . get_the_ID() . '"' . selected($page, get_the_ID(), false) . '>' . addslashes(get_the_title()) . '</OPTION>';
        //$list .= '<OPTION value="aa">aa</OPTION>';
...
$info4top = array('title'=>'', 'content'=>'', 'url'=>'', 'button_text'=>'', 'bzb_ruby'=>'', 'bzb_catch'=>'', 'button_text'=>'', 'button_url'=>'', 'image'=>'', 'service'=>'');
    ...
    switch($syubetu){
      case 'category' :
        ...
        $info4top['title'] = $cat_option['bzb_meta_title'];
        ...
        $image = isset($catt_option['cat_original_image']) ? $cat_option['cat_original_image'] : "";
        if(isset($image) && $image !== ''){
          $info4top['image'] = $image;
        }else{
          $info4top['image'] = get_stylesheet_directory_uri() . '/lib/images/fotolia_51633892.jpg';
        }
      break;
      ...
      case 'post' :
        ...
        $image = wp_get_attachment_image_src( $thumbnail_id, 'big_thumbnail' );
        if(isset($image[0]) && $image[0] !== ''){
          $info4top['image'] = $image[0];
        }else{
          $info4top['image'] = get_stylesheet_directory_uri() . '';
        }
      break;
...
function footer_social_buttons(){
  ...
  $footer_social_buttons .=  '<ul>';
          ...
          $footer_social_buttons .= '<li class="header-feedly"><a target="_blank" href="http://cloud.feedly.com/#subscription%2Ffeed%2F' . $feedly_url . '"><i class="fa fa-rss-square"></i>Feedly</a></li>';
        $footer_social_buttons .= '</ul>';
