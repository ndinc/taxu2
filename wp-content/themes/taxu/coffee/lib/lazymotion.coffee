class LazyMotion
  constructor: (options)->
    ...
    @$el = if @options.$target then @options.$target else $(@options.$el).find('['+@options.attr+']')
    @attr = @options.attr
    @cls = @options.cls
  ...
  setMoved: ->
    scrollTop = $(window).scrollTop()
    $.each @elems, (i, elem) =>
      elem.moved = !@isMoved(elem, scrollTop)
      return


  isMoved: (elem, scrollTop)->
